...
func (s *DashboardService) GetDashboardStats(userID uint, role string, perusahaanID uint, divisiID uint) (*DashboardStats, error) {
	...
	if err := taskScopeQuery(role, perusahaanID, divisiID, userID).Count(&totalTasks).Error; err != nil {
		return nil, err
	}

	var totalActionPlans int64
	if err := actionPlanScopeQuery(role, perusahaanID, divisiID, userID).Count(&totalActionPlans).Error; err != nil {
		return nil, err
	}
...
// dan yang sudah Done, dalam lingkup akses role yang sama dengan actionPlanScopeQuery.
